# Etc. Packing List
<style>
.avatar {
  width: 40px;
  height: 40px;
  border-radius: 50%;
  vertical-align: middle;
  margin-right: 8px;
  object-fit: cover;
}
</style>
[⬅️Back](README.md)

According to Anne, here is a list of suggestions to add to your luggage, with some editorials added by Matt:

* [ ] FLASHLIGHT. Dry alkaline batteries (like AA, AAA, C, D) are permitted in both carry-on and checked baggage worldwide. Not so much for the boat, but for your hotel stays in the hottest summer Europe has seen in decades.
* [ ] Magnetic hooks.
* [ ] Tissues/Travel toilet wipes.
* [ ] Hand sanitizer. Like all liquids, 3.4 oz limit or it has to be checked.
* [ ] Type F or C power converter/adapter for Barcelona.
* [ ] Type L or C power converter/adapter for Rome.
* [ ] The ship uses US plugs. Ask for an extension cord unless you really read the fine print on the NCL website and get an allowed cord.
* [ ] Univeral power adapter.
* [ ] Ziploc bags (two gallon, one gallon, one quart).
* [ ] Two nanobags (backpack and sling).
* [ ] Post-It notes and pen (to communicate with room steward).
* [ ] Highligher.
* [ ] Lanyard or yo-yo for cruise card.
* [ ] Closeline with clips.
* [ ] One insulated water bottle and carbiner.
* [ ] Cream Coach crossbody.
* [ ] iPad.
* [ ] Noise cancelling headphones (that can last seven hours between charges).
* [ ] Fan for the cabin.
